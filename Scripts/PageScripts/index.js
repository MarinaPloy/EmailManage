...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    document.querySelectorAll('#txtCnum,#txtSnum,#txtCname').forEach(el => {
        el.addEventListener('keydown', e => {
            if (e.key === 'Enter') {
                e.preventDefault();
                // Clear detail and email grid when user triggers search with Enter
                clearDetailAndEmail();
                ck++;
                loadData();
            }
    ...
    document.getElementById('btnSearch').addEventListener('click', function () {
        // Clear detail section and email table before running search
        clearDetailAndEmail();
        ck += 1;
        loadData();
    });
...
// New: clear detail inputs and clear email grid (used when clicking Search)
function clearDetailAndEmail() {
    // Clear detail form inputs
    document.querySelectorAll("#singleRecordForm input.form-control").forEach(i => i.value = '');

    // Hide Add Email button if present
    const addEmailBtn = document.getElementById("btnAddEmail");
    if (addEmailBtn) addEmailBtn.hidden = true;

    // Clear email grid rows
    if ($('#gridSearchEmail').data('datagrid')) {
        $('#gridSearchEmail').datagrid('loadData', { total: 0, rows: [] });
    }

    // Clear any selection in the customer grid so detail stays empty until user clicks a row
    if ($('#gridSearchCus').data('datagrid')) {
        try {
            $('#gridSearchCus').datagrid('clearSelections');
            if ($('#gridSearchCus').datagrid('clearChecked')) $('#gridSearchCus').datagrid('clearChecked');
        } catch (e) {
            // ignore if datagrid doesn't support clearChecked
        }
    }
}
...
function getTableEmail() {
    $('#gridSearchEmail').datagrid({
        ...
        columns: [[
            ...
            { field: 'email_taxinv', title: 'Tax Invoice Email', width: '70%', align: 'left', sortable: true },
...
function saveEmail() {
    ...
    // ตรวจสอบความยาวรวมของ email_taxinv ไม่ให้เกิน 250 ตัวอักษร
    const emailTaxInv = emails.join(';');
    if (emailTaxInv.length > 250) {
        const msg = 'อีเมลสำหรับ Tax Invoice ต้องไม่เกิน 250 ตัวอักษร';
        if (typeof Swal !== 'undefined' && typeof Swal.fire === 'function') {
            Swal.fire({ icon: 'error', title: 'แจ้งเตือน', text: msg });
        } else if (typeof swal === 'function') {
            swal('แจ้งเตือน', msg, 'error');
        } else {
            alert(msg);
        }
        return;
